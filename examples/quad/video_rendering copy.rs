...
use hal::{
    ...
    command,
    format as f,
    format::{AsFormat, ChannelType, R8Unorm as ColorFormat, Swizzle},
    image as i,
    memory as m,
    pass,
    pass::Subpass,
...
pub fn createRenderer(width: u32, height: u32)  -> Renderer<hal::Backend> {
    env_logger::init();
    ...
    let wb = winit::window::WindowBuilder::new()
        ...
        .with_title("Video Player".to_string());
    ...
    let mut renderer = Renderer::new(Some(instance), surface, adapter, window, event_loop, window::Extent2D { width, height });
...
pub fn createRenderer(width: u32, height: u32) -> Renderer<back> {
    ...
    let adapter = adapters.remove(0);
    let mut renderer = Renderer::new(None, surface, adapter, window, event_loop, window::Extent2D { width, height });
    ...
    B: hal::Backend,
{
    fn new(
        ...
        let buffer_stride = mem::size_of::<shader::Vertex>() as u64;
        let buffer_len = shader::QUAD.len() as u64 * buffer_stride;
        ...
        let buffer_memory = unsafe {
            ...
            ptr::copy_nonoverlapping(shader::QUAD.as_ptr() as *const u8, mapping, buffer_len as usize);
            device
        ...
        let yuv_data = frame_generator::generate_frame(width, height, 0);

        height = height * 3 / 2;

        let kind = i::Kind::D2(width as i::Size, height as i::Size, 1, 1);
        let row_alignment_mask = limits.optimal_buffer_copy_pitch_alignment as u32 - 1;
        let image_stride = 1usize;
        ...
        let image_upload_memory = unsafe {
            ...
            for y in 0 .. height as usize {
                let row = &(*yuv_data)[y * (width as usize) * image_stride
            ...
            let pipeline = {
                ...
                let vertex_buffers = vec![
                    pso::VertexBufferDesc {
                        binding: 0,
                        stride: mem::size_of::<shader::Vertex>() as u32,
